...
def UseExternalEnv(env, name):
    if os.environ.has_key(name):
        PrependEnvPath(env, name, os.environ[name])

def PrependEnvPath(env, name, *args):
    if env["ENV"].has_key(name):
        old = env["ENV"][name].split(":")
    else:
        old = []
    new = []
    for arg in args:
        new.extend(arg.split(":"))
    paths = list(new)
    for path in old:
        if path not in paths:
            paths.append(path)
    env["ENV"][name] = ":".join(paths)
...
def MakeEnvironment(default_prefix="/usr/local", prefix_is_home=False):
    ...
    _environment.AddMethod(UseExternalEnv, "UseExternalEnv")
    _environment.AddMethod(PrependEnvPath, "PrependEnvPath")
