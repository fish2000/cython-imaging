...
struct LayoutToNumpyType {
    ...
    void operator()(Field<T,0> const & field) const {
        dict[field.name] = boost::python::make_tuple(
            boost::numpy::dtype::get_builtin<T>(),
            field.offset
        );
    }
...
struct NumpyTypeToLayout {
    ...
    void operator()(Field<T,0> & field) const {
        ...
        boost::numpy::dtype pyElement 
            = boost::python::extract<boost::numpy::dtype>(s[0]);
        if (pyElement != boost::numpy::dtype::get_builtin<T>()) {
    ...
    void operator()(Field<T,N> & field) const {
        ...
        boost::numpy::dtype pyElement 
            = boost::python::extract<boost::numpy::dtype>(s2[0]);
        if (pyElement != boost::numpy::dtype::get_builtin<T>()) {
            PyErr_Format(PyExc_TypeError, "Type mismatch encountered in field %s.", field.name.c_str());
            boost::python::throw_error_already_set();
        }
    ...
    NumpyTypeToLayout(boost::numpy::dtype const & input) :
...
template <typename T>
boost::numpy::dtype makeNumpyType(Layout<T> const & layout) {
    LayoutToNumpyType function;
    boost::fusion::for_each(layout.getSequence(), function);
    return boost::numpy::dtype(function.dict, false);
}

template <typename T>
void fillLayout(boost::numpy::dtype const & input, Layout<T> & layout) {
